...
void Gui_DrawFont_GBK16_line(u8 line, u16 fc, u16 bc, u8 *s)
{
  ...
  //把字符点阵信息输出到缓冲中
	while(*s)
	{
      ...
      for(j=0;j<8;j++)
    	{
        if(asc16[k*16+i]&(0x80>>j)) DrawPointToBuff(x+j, i, fc);
        else  DrawPointToBuff(x+j, i, bc);
    	}
      x+=8;
      s++;
		}
		else
		{
			...
			for (k=0;k<hz16_num;k++)
			{
              ...
              for(j=0;j<8;j++)
							{
							    if(hz16[k].Msk[i*2]&(0x80>>j)) DrawPointToBuff(x+j, i, fc);
                  else  DrawPointToBuff(x+j, i, bc);
							}

              for(j=0;j<8;j++)
							{
                if(hz16[k].Msk[i*2+1]&(0x80>>j)) DrawPointToBuff(x+8+j, i, fc);
                else  DrawPointToBuff(x+8+j, i, bc);
							}
				    }
				}
			}
